	...
	public String showAllpurchases(Model model)
	{
		...
		if(purchaseList!=null)
		{
			for(Purchase_Entry p : purchaseList)
			{
				...
				try {
					  p1=productDao.findById(p.getPurchased_productid()).get();
				}catch(Exception e)
				{
					System.out.println("Exception Genrated Due to Prodcut with ID :: "+p.getPurchased_productid()+" Has been Removed or Doesn't exist in the Product Table any Longer");
					p1.setProduct_name("PRODUCT HAS BEEN REMOVED OR NO LONGER EXISTS IN DATABASE PRODUCT TABLE");
				}
	...
	public String searchpurchase(Model model,@RequestParam("method") String method,HttpServletRequest request,HttpServletResponse response)
	{
		...
		if(method.equals("particulardate")) {
			...
			for(Purchase_Entry pe:purchasesearchresultlist)
			{
				...
				Product p1 = new Product();
				try {
					  p1=productDao.findById(pe.getPurchased_productid()).get();
				}catch(Exception e)
				{
					System.out.println("Exception Genrated Due to Prodcut with ID :: "+pe.getPurchased_productid()+" Has been Removed or Doesn't exist in the Product Table any Longer");
					p1.setProduct_name("PRODUCT HAS BEEN REMOVED OR NO LONGER EXISTS IN DATABASE PRODUCT TABLE");
				}
