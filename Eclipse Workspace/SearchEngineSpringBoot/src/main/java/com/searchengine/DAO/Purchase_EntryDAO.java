...
import com.searchengine.Models.Purchase_Entry;
...
public interface Purchase_EntryDAO extends CrudRepository<Purchase_Entry, Integer>{
	...
	@Query(value="select * from purchase_entry where purchased_date like :date",nativeQuery = true)
	...
	@Query(value = "select product_name as searchlist from product where product_name like :keyword Union select username from user where username like :keyword Union select product_type from product where product_type like :keyword Union Select phonenumber from user where phonenumber like :keyword order by searchlist",nativeQuery = true)
	public List<String> searchkeyword(@Param("keyword") String keyword);
